...
// Abrir URL en el navegador por defecto (cross-platform)
function openUrlInBrowser(url) {
  ...
  // no intentar abrir en entornos CI/headless
  if (process.env.CI || process.env.GITHUB_ACTIONS || process.env.CODESPACES)
    return false;

  try {
    if (process.platform === "win32") {
      // start requiere un título vacío "" para que funcione con comillas en la URL
      shell.exec(`start "" "${url}"`, { silent: true });
    } else if (process.platform === "darwin") {
      shell.exec(`open "${url}"`, { silent: true });
    } else {
      // linux u otros -> xdg-open (muy común)
      shell.exec(`xdg-open "${url}"`, { silent: true });
    }
    return true;
  } catch (e) {
    // no romper la ejecución si falla
    return false;
  }
}
  ...
  openrouter: {
    ...
    models: [
      ...
      { id: "openai/gpt-4o-mini", label: "GPT-4o Mini (rápido)" },
      {
        ...
        label: "Claude 3.5 Sonnet (excelente)",
      },
...
async function createProjectFiles(cwd, provider, apiKey, promptText, projectName) {
  ...
  fs.writeFileSync(
    ...
    `${providerConfig.envVar}=${cleanApiKey}`,
    { encoding: "utf8" }
  );

  const safePrompt = promptText
    ? promptText.replace(/"/g, '\\"')
    : "Experto asesor en servicio al cliente";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log("Chat API [${
      providerConfig.name
    }] - model:", chosenModel, "key-prefix:", (process.env.${
    ...
    headers["HTTP-Referer"] = "https://jarc-ai.vercel.app";
    headers["X-Title"] = "JARC AI";`
        : ""
    }
...
`;
  ...
  const modelsJsArray = providerConfig.models
    .map((m) => `  { id: "${m.id}", label: "${m.label}" }`)
    .join(",\n");

  // 🔧 CORRECCIÓN: Obtener el nombre del proyecto dinámicamente
...
export default function Home() {
  ...
  const [projectName, setProjectName] = useState("Mi Agente");

  // 🔧 CORRECCIÓN: Obtener nombre del proyecto dinámicamente
  useEffect(() => {
    if (typeof window !== 'undefined') {
      let name = "${projectName}"; // Fallback al nombre generado
      
      // Si estamos en Vercel, extraer solo la primera parte del hostname
      if (window.location.hostname.includes('vercel.app')) {
        const hostname = window.location.hostname;
        // Para URLs como: test5-7k6nj906b-jarcvon663-8369s.vercel.app
        // Tomamos solo la primera parte antes del primer guión
        const firstPart = hostname.split('-')[0];
        if (firstPart && firstPart !== 'www') {
          name = firstPart.charAt(0).toUpperCase() + firstPart.slice(1);
        }
      } else if (window.location.hostname !== 'localhost') {
        // Para otros dominios, tomar el nombre antes del primer punto
        const domainParts = window.location.hostname.split('.');
        if (domainParts[0] && domainParts[0] !== 'www') {
          name = domainParts[0].charAt(0).toUpperCase() + domainParts[0].slice(1);
        }
      }
  ...
  return (
    <div className="chat-container">
      ...
      <div className="footer">
        <span>Creado con ❤️ por <strong>JARC-AI</strong></span>
      </div>
    </div>
  );
}
...
async function deployToVercel(cwd, projectName, provider, apiKey) {
  ...
  if (first.code !== 0) {
    return {
      ok: false,
      error: "vercel_deploy_failed_initial",
      detail: first.stdout || first.stderr,
    };
  }
  ...
  if (VERCEL_TOKEN) {
    ...
    try {
      ...
      if (!projectsRes.ok) {
        ...
        console.log(
          chalk.yellow("⚠️ No se pudo obtener lista de proyectos en Vercel:"),
          txt
        );
      } else {
        ...
        if (!project) {
          console.log(
            chalk.yellow(
              `⚠️ Proyecto '${projectName}' no encontrado en Vercel.`
            )
          );
        } else {
          ...
          if (!r.ok) {
            ...
            console.log(
              chalk.yellow("⚠️ No se pudo crear/actualizar la env via API:"),
              txt
            );
          } else {
            console.log(
              chalk.green(
                `✅ Variable ${providerConfig.envVar} configurada en Vercel via API.`
              )
            );
            envOk = true;
          }
      ...
      console.log(
        chalk.yellow("⚠️ Falló al intentar crear env en Vercel via API:"),
        e.message
      );
    }
  } else {
    ...
    try {
      ...
      if (res.code === 0) {
        console.log(
          chalk.green(
            `✅ Variable ${providerConfig.envVar} configurada en Vercel via CLI.`
          )
        );
        envOk = true;
      } else {
        console.log(
          chalk.yellow("⚠️ No se pudo configurar la variable con la CLI.")
        );
      }
    } catch (e) {
  ...
  if (envOk) {
    ...
    const redeployCmd = VERCEL_TOKEN
...
async function run() {
      ...
      choices: [
        {
          name: `${chalk.cyan(
            "Groq"
          )} - Ultra rápido, modelos Llama (económico)`,
          value: "groq",
        },
        {
          name: `${chalk.green(
            "OpenRouter"
          )} - GPT-4, Claude, Gemini y más (versátil)`,
          value: "openrouter",
        },
      ],
    },
  ...
  console.log(
    chalk.blue(`   Modelos disponibles: ${providerConfig.models.length}`)
  );
  ...
  createProjectFiles(cwd, provider, apiKey, promptText, projectName);
  ...
  const commitRes = shell.exec(
    `git commit -m "feat: scaffold JARC-AI with ${providerConfig.name}"`,
    { silent: true }
  );
  if (commitRes.code !== 0) {
    shell.exec('git config user.email "jarcvon663@gmail.com"', {
      silent: true,
    });
    shell.exec('git config user.name "JARC-AI"', { silent: true });
    shell.exec("git add .", { silent: true });
    shell.exec(
      `git commit -m "feat: scaffold JARC-AI with ${providerConfig.name}"`,
      {
  ...
  if (!vercelRes.ok) {
    ...
    if (vercelRes.url) {
      console.log(
        chalk.cyan(
          `🔗 URL (puede necesitar configuración manual): ${vercelRes.url}`
        )
      );
    }
    console.log(
    ...
    if (vercelRes.url) {
      ...
      const opened = openUrlInBrowser(vercelRes.url);
      if (opened) {
        console.log(chalk.green("✅ Abriendo navegador con la URL..."));
      } else {
        console.log(
          chalk.yellow(
            "⚠️ No se pudo abrir el navegador automáticamente (entorno headless o fallo)."
          )
        );
      }
    }
  ...
  console.log(
    chalk.white(`   🧠 Modelos: ${providerConfig.models.length} disponibles`)
  );
  ...
  console.log(
    chalk.gray("🛡️ Nunca compartas tu .env.local en repositorios públicos")
  );
}
